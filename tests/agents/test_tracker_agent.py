...
def _make_db(
    *,
    ...
    # .select("status").eq(...).limit(1).execute()  → used by transition()
    select_status_chain = MagicMock()
    select_status_chain.eq.return_value.limit.return_value.execute.return_value.data = (
        [app_row] if app_row is not None else []
    )
    app_mock.select.return_value = select_status_chain

    # .update(...).eq(...).execute()  → used by transition() and check_follow_ups()
    update_chain = MagicMock()
    update_chain.eq.return_value.execute.return_value.data = []
    app_mock.update.return_value = update_chain

    # .delete().neq(...).execute()  → used by delete_my_data()
    delete_chain = MagicMock()
    delete_chain.neq.return_value.execute.return_value.data = (
...
def _make_db_for_follow_ups(rows: list[dict]) -> MagicMock:
    ...
    Query chain used in check_follow_ups:
        .select(...).eq("status","applied").is_("follow_up_at","null")
            .not_.is_("applied_at","null").execute()
    """
    db = MagicMock()
    app_mock = MagicMock()

    # Chain: select -> eq -> is_ -> not_.is_ -> execute
    not_chain = MagicMock()
    ...
    app_mock.select.return_value.eq.return_value = eq_chain
...
def test_valid_transitions(from_status: str, to_status: str) -> None:
    ...
    agent.transition("app-1", to_status)
...
def test_invalid_transition_message_contains_statuses() -> None:
    ...
    with pytest.raises(InvalidTransitionError, match="new") as exc_info:
        agent.transition("app-1", "rejected")
...
def test_transition_to_applied_sets_applied_at() -> None:
    ...
    agent = TrackerAgent(db=db, clock=_fixed_clock)
    agent.transition("app-1", "applied")
...
def test_transition_does_not_set_applied_at_for_other_statuses() -> None:
    ...
    agent = TrackerAgent(db=db, clock=_fixed_clock)
    agent.transition("app-1", "interview")
...
def test_transition_missing_id_raises_lookup_error() -> None:
    ...
    with pytest.raises(LookupError, match="app-missing"):
        agent.transition("app-missing", "ready_to_send")
...
def test_check_follow_ups_sets_follow_up_at_for_old_rows() -> None:
    ...
    updated = agent.check_follow_ups()
...
def _make_db_for_delete(
    *,
    ...
    # .select(...).execute()  → returns file-path rows
    select_chain = MagicMock()
    select_chain.execute.return_value.data = app_file_rows
    app_mock.select.return_value = select_chain

    # .delete().neq(...).execute()
    delete_chain = MagicMock()
    delete_chain.neq.return_value.execute.return_value.data = apps_deleted
    app_mock.delete.return_value = delete_chain
    ...
    p_delete_chain.neq.return_value.execute.return_value.data = profiles_deleted
    profile_mock.delete.return_value = p_delete_chain
    db._profile_mock = profile_mock

    match_mock = MagicMock()
    m_delete_chain = MagicMock()
    m_delete_chain.neq.return_value.execute.return_value.data = matches_deleted
...
def test_delete_my_data_deletes_all_three_tables(tmp_path: Path) -> None:
    db = _make_db_for_delete(
        app_file_rows=[],
    ...
    # All three delete chains were called
